...
CREATE TABLE user (
    ...
    preferences TEXT,
    CONSTRAINT fk_user_account FOREIGN KEY (account_uuid) REFERENCES account(uuid) ON DELETE CASCADE,
...
CREATE TABLE channel_member (
    ...
    CONSTRAINT unique_channel_member UNIQUE(channel_uuid, user_uuid),
    CONSTRAINT fk_member_channel FOREIGN KEY (channel_uuid) REFERENCES channel(uuid) ON DELETE CASCADE,
    CONSTRAINT fk_member_user FOREIGN KEY (user_uuid) REFERENCES account(uuid) ON DELETE CASCADE,
...
CREATE TABLE channel_follower (
    ...
    CONSTRAINT fk_follower_user FOREIGN KEY (follower_uuid) REFERENCES account(uuid) ON DELETE CASCADE
);
...
CREATE TABLE event_registration (
    ...
    CONSTRAINT fk_registration_user FOREIGN KEY (user_uuid) REFERENCES user(account_uuid) ON DELETE CASCADE
);
...
CREATE TABLE event_feedback (
    ...
    CONSTRAINT fk_feedback_user FOREIGN KEY (user_uuid) REFERENCES user(account_uuid) ON DELETE CASCADE
);

CREATE TABLE chat_history (
    uuid UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    user_uuid UUID NOT NULL,
    session_id UUID DEFAULT uuid_generate_v7(),
    ...
    CONSTRAINT fk_chat_user FOREIGN KEY (user_uuid) REFERENCES user(account_uuid) ON DELETE CASCADE
);
...
CREATE TABLE recommendation (
    ...
    CONSTRAINT unique_recommendation UNIQUE(user_uuid, event_uuid),
    CONSTRAINT fk_rec_user FOREIGN KEY (user_uuid) REFERENCES user(account_uuid) ON DELETE CASCADE,
