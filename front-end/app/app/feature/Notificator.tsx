...
import { notificationSSE } from "@/hooks/sseManager";
import { API_BASE_URL } from "@/lib/api/clients";
import { notificationsApi } from "@/lib/api/notifications";
import { Notification } from "@/lib/types";
import { notificationTypeFilter } from "@/lib/utils";
...
export default function Notificator() {
  ...
  const handleMarkAsRead = async (notif: Notification) => {
    ...
    setNotifications((prev) => (
        prev.map(
            (n) => n.id === notif.id? {...n, isRead: true}: n
        )
    ))
  };
  ...
  useEffect(() => {
    ...
    notificationSSE.connect(
      ...
      (data) => {
        const notif = notificationTypeFilter(data);
        if (notif) {
  ...
  async function load() {
    ...
    const mapped = items.map(notificationTypeFilter).filter((n): n is Notification => n !== null);
      ...
      {isNotifOpen && (
        <div className="absolute top-full right-0 mt-4 w-80 sm:w-96 bg-white dark:bg-[#121212] border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl z-40 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/5">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="cursor-pointer text-xs text-brand-purple hover:text-purple-600 font-medium"
              >
                Mark all read
              </button>
            )}
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((notif) => (
                <div
                  ...
                  key={notif.id}
                  className={`cursor-pointer p-4 border-b border-gray-100 dark:border-white/5 last:border-none hover:bg-gray-50 dark:hover:bg-white/5 transition-colors ${
          ...
          <div className="p-2 border-t border-gray-100 dark:border-white/5 text-center">
            <button className="text-xs font-medium text-gray-500 hover:text-black dark:hover:text-white transition-colors">
              {/* View All History */}
            </button>
          </div>
        </div>
