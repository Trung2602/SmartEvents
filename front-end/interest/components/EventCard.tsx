...
const EventCard: React.FC<EventCardProps> = ({ event, variant = 'landing', onClick, onToggleInterest }) => {
  const isDashboard = variant === 'dashboard';

  return (
    <div 
        onClick={onClick}
        className={`group flex flex-col w-full bg-white dark:bg-[#121212] rounded-3xl overflow-hidden cursor-pointer transition-all duration-500 ${
            isDashboard 
    ...
    >
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2">
           {isDashboard && (
             <button 
                ...
                className={`p-2 rounded-full backdrop-blur-md border transition-all duration-300 hover:scale-110 ${event.isLiked ? 'bg-red-500/90 border-red-500 text-white' : 'bg-black/30 border-white/10 text-white hover:bg-black/50'}`}
             >
      ...
      <div className="p-5 flex flex-col gap-3 flex-1 relative">
         ...
         <div className="flex items-center justify-between pt-4 mt-2 border-t border-gray-100 dark:border-white/5">
             ...
             {isDashboard && event.attendeesCount && (
