...
const TimelineEventList: React.FC<TimelineEventListProps> = ({ events, onSelect, onToggleInterest }) => {
  ...
  return (
    <div className="pl-0 md:pl-4 space-y-8">
      {Object.entries(groupedEvents).map(([date, groupEvents]: [string, Event[]]) => {
        ...
        return (
          <div key={date} className="flex gap-4 md:gap-8 relative group/date">
            
            {/* Left Column: Date (Sidebar) - Desktop Only */}
            <div className="w-16 md:w-24 flex-shrink-0 text-right pt-2 hidden md:block">
            ...
            <div className="flex-1 min-w-0 space-y-6">
               ...
               {groupEvents.map(event => (
                  <div 
                    ...
                    className="group relative flex flex-col md:flex-row h-auto md:h-64 w-full bg-white dark:bg-[#121212] border border-gray-100 dark:border-white/5 rounded-3xl overflow-hidden hover:shadow-xl hover:border-gray-200 dark:hover:border-white/10 transition-all duration-300 cursor-pointer"
                  >
                     {/* Left: Image Section (Full Cover, approx 40%) */}
                     <div className="w-full md:w-[40%] h-48 md:h-full relative overflow-hidden shrink-0">
                        <img 
                          src={event.imageUrl} 
                     ...
                     <div className="flex-1 relative p-6 flex flex-col justify-center gap-1.5 overflow-hidden">
                        ...
                        {/* Using a pseudo-element logic via absolute div to create a colored ambient background based on the image */}
                        <div 
                        ...
                        {/* Overlay to ensure text readability - lighter in light mode, darker in dark mode */}
                        ...
                        <div className="relative z-10 flex flex-col gap-2 pr-12">
                           ...
                           <div className="flex items-center gap-2">
                             <img src={event.organizerAvatar} alt={event.organizerName} className="w-6 h-6 rounded-full object-cover border border-white/20" />
                             <span className="text-sm text-gray-600 dark:text-gray-300 font-medium truncate">
                               Hosted by <span className="text-gray-900 dark:text-white font-semibold">{event.organizerName}</span>
                             </span>
                           </div>
                           ...
                           <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                               ...
                               <span className="font-medium truncate">{event.dateStr} • {event.startTime} - {event.endTime}</span>
                           </div>
                        ...
                        <button 
                            onClick={(e) => { e.stopPropagation(); onToggleInterest(event.id); }} 
                            className={`absolute top-6 right-6 p-2 rounded-full transition-all z-20 ${event.isLiked ? 'bg-red-50 text-red-500 shadow-sm' : 'bg-transparent text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-red-500'}`}
                        >
                            <Heart size={24} fill={event.isLiked ? "currentColor" : "none"} />
                        </button>

                        {/* Bottom Right: Price */}
                        <div className="absolute bottom-6 right-6 z-20">
                           <span className={`text-lg font-bold px-4 py-2 rounded-xl border ${event.price === 'Free' ? 'bg-green-50 border-green-100 text-green-700 dark:bg-green-900/10 dark:border-green-900/30 dark:text-green-400' : 'bg-white border-gray-200 text-gray-900 dark:bg-white/5 dark:border-white/10 dark:text-white shadow-sm'}`}>
                             {event.price}
                           </span>
                        </div>
                     </div>
